defmodule PodiumWeb.VideoLive.FormComponent do
  ...
  def update(%{video: video} = assigns, socket) do
    changeset = Recording.change_video(video)
  ...
  def handle_event("validate", %{"video" => video_params}, socket) do
    changeset =
      ...
      |> Recording.change_video(video_params)
  ...
  defp save_video(socket, :edit, video_params) do
    case Recording.update_video(socket.assigns.video, video_params) do
      {:ok, _video} ->
        {:noreply,
         socket
  ...
  defp save_video(socket, :new, video_params) do
    case Recording.create_video(video_params) do
      {:ok, _video} ->
        {:noreply,
         socket
